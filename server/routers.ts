...
export const appRouter = router({
  ...
  audits: router({    
    ...
    getCalendarEvents: protectedProcedure
      .input(z.object({ tenantId: z.number() }))
      .query(async ({ input }) => {
        return db.getAuditCalendarEvents(input.tenantId);
      }),
    ...
    createCalendarEvent: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        // Get audit type and location names
        const auditType = await db.getAuditTypeById(input.auditTypeId);
        const location = await db.getLocationById(input.locationId);
        
        if (!auditType || !location) {
          throw new Error("Audit type or location not found");
        }

        return db.createAuditCalendarEvent({
          tenantId: input.tenantId,
          locationId: input.locationId,
          auditTypeId: input.auditTypeId,
    ...
    deleteCalendarEvent: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        return db.deleteAuditCalendarEvent(input.eventId, ctx.user.tenantId);
      }),

    autoSuggestAuditDates: protectedProcedure
      .input(z.object({ tenantId: z.number() }))
      .mutation(async ({ input, ctx }) => {
        // Get all audit types and locations
        const auditTypes = await db.getAllAuditTypes();
        const locations = await db.getLocationsByTenant(input.tenantId);
        
        let created = 0;
        const now = new Date();
        
        // For each audit type and location, create monthly audits for next 12 months
        for (const auditType of auditTypes) {
          for (const location of locations) {
            // Create 12 monthly audits, distributed across the month (mid-month)
            for (let month = 0; month < 12; month++) {
              const scheduledDate = new Date(now);
              scheduledDate.setMonth(now.getMonth() + month);
              scheduledDate.setDate(15); // Mid-month for flexibility
              scheduledDate.setHours(10, 0, 0, 0); // 10 AM
              
              await db.createAuditCalendarEvent({
                tenantId: input.tenantId,
